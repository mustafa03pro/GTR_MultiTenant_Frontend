...
const BillDetailsReport = () => {
  ...
  const getDatesFromRange = (range) => {
    ...
    switch(range) {
      case 'Today':
        break; 
      case 'This Week': // Start of week to end of week? Or Start to Today? Let's do Start to End of Week (Sat/Sun)
      ...
      case 'This Month':
        from.setDate(1); 
        to.setMonth(today.getMonth() + 1, 0); // End of month
        break; 
      case 'This Quarter':
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      if (!from || !to || (dateRange === 'Custom' && (!customFromDate || !customToDate))) {
           if (dateRange !== 'Custom') return; // Should allow custom to wait for input
      }

      const formatDate = (d) => d ? d.toISOString().split('T')[0] : '';
      if (dateRange === 'Custom' && (!customFromDate || !customToDate)) return;
      ...
      const res = await axios.get(`${API_URL}/purchases/reports/bill-details`, {
  ...
  // Totals
  const totalBillAmount = filteredData.reduce((sum, item) => sum + (item.billAmount || 0), 0);
  const totalBalanceAmount = filteredData.reduce((sum, item) => sum + (item.balanceAmount || 0), 0);
    ...
    <div className="p-4 md:p-6 bg-slate-50 min-h-screen font-sans text-sm">
      ...
      <div className="bg-white p-4 rounded-b-md shadow-sm border border-slate-200 mb-6">
        ...
        <div className="text-center mt-6 mb-2">
            ...
            <h3 className="text-md font-bold text-slate-700 uppercase">BILL DETAILS</h3>
                  ...
                  <thead>
                      <tr className="bg-[#ead1dc] text-slate-800 font-semibold border-b border-white">
                          {[
                            ...
                            { label: 'Bill Amount', key: 'billAmount', align: 'right' },
                            { label: 'Balance Amount', key: 'balanceAmount', align: 'right' }
                          ].map((col) => (
                              <th 
                                key={col.key} 
                  ...
                  <tbody className="divide-y divide-slate-100">
                      {paginatedData.length > 0 ? paginatedData.map((item, index) => (
                          <tr key={index} className="hover:bg-slate-50">
                              <td className="px-3 py-2 border-r border-slate-100">{item.status}</td>
                              <td className="px-3 py-2 font-medium text text-slate-700 border-r border-slate-100">{item.billNumber}</td>
                              <td className="px-3 py-2 border-r border-slate-100">{formatDateDisplay(item.billDate)}</td>
                              <td className="px-3 py-2 border-r border-slate-100">{formatDateDisplay(item.dueDate)}</td>
                              <td className="px-3 py-2 border-r border-slate-100">{item.referenceNumber}</td>
                              <td className="px-3 py-2 font-medium border-r border-slate-100">{item.supplierName}</td>
                              <td className="px-3 py-2 text-right text-red-600 font-medium border-r border-slate-100">
                      ...
                      {filteredData.length > 0 && (
                          <tr className="font-bold bg-slate-50 border-t border-slate-300">
                              ...
                              <td className="px-3 py-2 text-right">{formatCurrency(totalBillAmount)}</td>
                              <td className="px-3 py-2 text-right">{formatCurrency(totalBalanceAmount)}</td>
                          </tr>
                      )}
                  </tbody>
              </table>
