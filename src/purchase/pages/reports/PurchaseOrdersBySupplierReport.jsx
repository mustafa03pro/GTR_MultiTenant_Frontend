...
const PurchaseOrdersBySupplierReport = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_URL}/purchases/reports/purchase-orders-by-supplier`, {
  ...
  // Totals
  const totalPurchaseOrdersCount = filteredData.reduce((sum, item) => sum + (item.purchaseOrdersCount || 0), 0);
    ...
    <div className="p-4 md:p-6 bg-slate-50 min-h-screen font-sans text-sm">
      ...
      <div className="flex flex-col gap-4 mb-6">
        ...
        <div className="bg-[#a65d8c] text-white px-4 py-3 rounded-t-md shadow-sm flex justify-between items-center">
            <h1 className="text-xl font-semibold">Purchase Orders by Supplier</h1>
      ...
      <div className="bg-white p-4 rounded-b-md shadow-sm border border-slate-200 mb-6">
        ...
        <div className="text-center mt-6 mb-2">
            ...
            <h3 className="text-md font-bold text-slate-700 uppercase">PURCHASE ORDERS BY SUPPLIER</h3>
                  ...
                  <thead>
                      <tr className="bg-[#ead1dc] text-slate-800 font-semibold border-b border-white">
                          {[
                            ...
                            { label: 'Purchase Orders Count', key: 'purchaseOrdersCount', align: 'right' },
                  ...
                  <tbody className="divide-y divide-slate-100">
                      {paginatedData.length > 0 ? paginatedData.map((item, index) => (
                          <tr key={index} className="hover:bg-slate-50 text-slate-700">
                              <td className="px-3 py-2 font-medium text-slate-700 border-r border-slate-100">{item.supplierName}</td>
                              <td className="px-3 py-2 text-right border-r border-slate-100">{item.purchaseOrdersCount}</td>
                      ...
                      {filteredData.length > 0 && (
                          <tr className="font-bold bg-slate-50 border-t border-slate-300 text-slate-800">
                              ...
                              <td className="px-3 py-2 text-right">{totalPurchaseOrdersCount}</td>
...
export default PurchaseOrdersBySupplierReport;
