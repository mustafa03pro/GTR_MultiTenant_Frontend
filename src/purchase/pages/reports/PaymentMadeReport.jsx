...
const PaymentMadeReport = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_URL}/purchases/reports/payments-made`, {
      ...
      console.error('Error fetching payment made report:', err);
  ...
  const filteredData = React.useMemo(() => {
    return sortedData.filter(item => 
      (item.paymentNumber && item.paymentNumber.toLowerCase().includes(searchTerm.toLowerCase())) ||
      ...
      (item.billNumbers && item.billNumbers.toLowerCase().includes(searchTerm.toLowerCase()))
    );
  ...
  const totalUnusedAmount = filteredData.reduce((sum, item) => sum + (item.unusedAmount || 0), 0);
    ...
    <div className="p-4 md:p-6 bg-slate-50 min-h-screen font-sans text-sm">
      ...
      <div className="bg-white p-4 rounded-b-md shadow-sm border border-slate-200 mb-6">
        ...
        <div className="text-center mt-6 mb-2">
            ...
            <h3 className="text-md font-bold text-slate-700 uppercase">PAYMENTS MADE</h3>
                  ...
                  <thead>
                      <tr className="bg-[#ead1dc] text-slate-800 font-semibold border-b border-white">
                          {[
                            ...
                            { label: 'Bill#', key: 'billNumbers' },
                            { label: 'Amount', key: 'amount', align: 'right' },
                            { label: 'Unused Amount', key: 'unusedAmount', align: 'right' }
                          ].map((col) => (
                              <th 
                                key={col.key} 
                  ...
                  <tbody className="divide-y divide-slate-100">
                      {paginatedData.length > 0 ? paginatedData.map((item, index) => (
                          <tr key={index} className="hover:bg-slate-50 text-slate-700">
                              <td className="px-3 py-2 font-medium text-[#a65d8c] border-r border-slate-100">{item.paymentNumber}</td>
                              <td className="px-3 py-2 border-r border-slate-100">{formatDateDisplay(item.paymentDate)}</td>
                              ...
                              <td className="px-3 py-2 border-r border-slate-100">{item.paymentMethod}</td>
                              <td className="px-3 py-2 border-r border-slate-100">{item.billNumbers}</td>
                              <td className="px-3 py-2 text-right font-medium border-r border-slate-100">
                                  {formatCurrency(item.amount)}
                              </td>
                              <td className="px-3 py-2 text-right font-medium">
                                  {formatCurrency(item.unusedAmount)}
                              </td>
                          </tr>
                      )) : (
                      ...
                      {filteredData.length > 0 && (
                          <tr className="font-bold bg-slate-50 border-t border-slate-300 text-slate-800">
                              ...
                              <td className="px-3 py-2 text-right">{formatCurrency(totalUnusedAmount)}</td>
                          </tr>
                      )}
                  </tbody>
              </table>
