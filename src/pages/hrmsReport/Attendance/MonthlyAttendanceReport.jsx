...
import { Download, Loader, Calendar, FileText, ShieldCheck, Search, AlertCircle } from 'lucide-react';
...
const MonthlyAttendanceReport = () => {
    ...
    const fetchAndParseReport = async () => {
        ...
        try {
            ...
            const response = await axios.get(`${API_URL}/reports/attendance/monthly`, {
            ...
            setError('Could not load attendance summary. You can still try to download the file.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            link.href = url;
            link.setAttribute('download', `Monthly_Attendance_Summary_${selectedMonth}.xlsx`);
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">Monthly Attendance Summary</h2>
                    <p className="text-sm text-gray-500">View and export aggregated attendance data.</p>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        ...
                        className="px-4 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-lg font-medium transition-colors flex items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed text-sm"
                    >
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-center gap-4 bg-gray-50 dark:bg-gray-900/50">
                    <h3 className="font-semibold text-gray-700 dark:text-gray-200">Summary - {selectedMonth}</h3>
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center p-12">
                        <Loader className="animate-spin h-8 w-8 text-teal-600 mb-4" />
                        <p className="text-gray-500">Loading attendance summary...</p>
                    </div>
                ) : error ? (
                    <div className="flex flex-col items-center justify-center p-12 text-center">
                        ...
                        <button onClick={fetchAndParseReport} className="mt-4 text-teal-600 hover:underline">Try Again</button>
                    </div>
                ) : (
                                ...
                                {filteredData.length > 0 ? (
                                    ...
                                    <tr>
                                        <td colSpan={tableData.length > 0 ? Object.keys(tableData[0]).length : 1} className="px-6 py-8 text-center text-gray-500">
                                            {tableData.length === 0 ? "No records found for this month." : "No records match your search."}
                                        </td>
                                    </tr>
                                )}
                            </tbody>
