...
const DailyAttendanceReport = () => {
    ...
    const fetchAndParseReport = async () => {
        ...
        try {
            ...
            const response = await axios.get(`${API_URL}/reports/attendance/daily`, {
            ...
            // Don't show error on 404/empty if it's just no data, but usually report returns empty excel
            setError('Could not load attendance preview. You can still try to download the file.');
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h2 className="text-xl font-bold text-gray-800 dark:text-gray-100">Daily Attendance Report</h2>
                    <p className="text-sm text-gray-500">View and export detailed attendance records.</p>
                </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
                <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex flex-col sm:flex-row justify-between items-center gap-4 bg-gray-50 dark:bg-gray-900/50">
                    <h3 className="font-semibold text-gray-700 dark:text-gray-200">Attendance Records - {selectedDate}</h3>
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col items-center justify-center p-12 text-center">
                        ...
                        <button onClick={fetchAndParseReport} className="mt-4 text-purple-600 hover:underline">Try Again</button>
                    </div>
                ) : (
                                ...
                                {filteredData.length > 0 ? (
                                    ...
                                    <tr>
                                        <td colSpan={tableData.length > 0 ? Object.keys(tableData[0]).length : 1} className="px-6 py-8 text-center text-gray-500">
                                            {tableData.length === 0 ? "No attendance records found for this date." : "No records match your search."}
                                        </td>
                                    </tr>
                                )}
                            </tbody>
