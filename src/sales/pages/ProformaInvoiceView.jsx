...
const ProformaInvoiceView = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const [invoiceRes, companyRes] = await Promise.all([
                    axios.get(`${API_URL}/sales/proforma-invoices/${id}`, { headers }),
    ...
    const handleDownloadPDF = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await axios.get(`${API_URL}/sales/proforma-invoices/pdf/${id}`, {
            ...
            link.href = url;
            link.setAttribute('download', `ProformaInvoice_${invoice.invoiceNumber}.pdf`);
    ...
    const handleDelete = async () => {
        if (window.confirm('Are you sure you want to delete this proforma invoice?')) {
            try {
                const token = localStorage.getItem('token');
                await axios.delete(`${API_URL}/sales/proforma-invoices/${id}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
        ...
        <div className="flex flex-col h-screen bg-slate-50 print:bg-white print:h-auto">
            ...
            <div className="bg-white border-b shadow-sm print:hidden">
                <div className="px-6 py-2 text-xs text-gray-500 flex items-center gap-1">
                    ...
                    <span className="font-medium text-gray-700">View Proforma Invoice</span>
                </div>
                <div className="px-6 py-3 bg-primary text-white flex justify-between items-center">
                    <h1 className="text-xl font-semibold">View Proforma Invoice</h1>
                    <div className="flex gap-2">
                        <button onClick={() => navigate('/sales/proforma-invoices')} className="px-3 py-1 bg-primary hover:bg-violet-800 text-sm rounded transition-colors">All Invoices</button>
                        <button onClick={() => navigate('/sales/proforma-invoices/new')} className="px-3 py-1 bg-primary hover:bg-violet-800 text-sm rounded transition-colors">+ New Invoice</button>
                    </div>
                </div>
            </div>
                ...
                <div className="w-full bg-white border shadow-sm p-8 relative text-sm print:w-full print:border-none print:shadow-none print:p-0 min-h-[1000px]">
                    ...
                    <div className="flex justify-between items-start border-t border-b border-gray-300 py-4 mb-6">
                        <div className="space-y-1">
                            ...
                            <div className="flex"><span className="w-32 font-semibold">PO Number</span><span>: {invoice.poNumber || '-'}</span></div>
                    ...
                    <div className="mb-6 bg-gray-100 p-2">
                        ...
                        <div className="text-gray-700">
                            ...
                            {/* Ideally we would have address here, for now use placeholder or fetch if critical */}
