...
import { ArrowLeft, Printer, FileText, Mail, Download, Edit, Trash2, ChevronDown, Paperclip, Loader2 } from 'lucide-react';
...
const InvoiceView = () => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const [invoiceRes, companyRes] = await Promise.all([
                    axios.get(`${API_URL}/sales/sales-invoices/${id}`, { headers }),
    ...
    const handleDelete = async () => {
        if (window.confirm('Are you sure you want to delete this invoice?')) {
            try {
                const token = localStorage.getItem('token');
                await axios.delete(`${API_URL}/sales/sales-invoices/${id}`, {
                    headers: { Authorization: `Bearer ${token}` }
                });
        ...
        <div className="flex flex-col h-screen bg-slate-50 print:bg-white print:h-auto">
            ...
            <div className="bg-white border-b shadow-sm print:hidden">
                <div className="px-6 py-2 text-xs text-gray-500 flex items-center gap-1">
                    ...
                    <span className="font-medium text-gray-700">View Invoice</span>
                </div>
                <div className="px-6 py-3 bg-primary text-white flex justify-between items-center">
                    <h1 className="text-xl font-semibold">View Invoice</h1>
                    <div className="flex gap-2">
                        <button onClick={() => navigate('/sales/invoices')} className="px-3 py-1 bg-primary hover:bg-violet-800 text-sm rounded transition-colors">All Invoices</button>
                        <button onClick={() => navigate('/sales/invoices/new')} className="px-3 py-1 bg-primary hover:bg-violet-800 text-sm rounded transition-colors">+ New Invoice</button>
                    </div>
                </div>
            </div>
            ...
            <div className="px-6 py-3 flex flex-wrap gap-2 items-center bg-white border-b print:hidden">
                <button onClick={() => navigate(`/sales/invoices/edit/${id}`)} className="p-2 bg-[#5bc0de] text-white rounded hover:bg-[#46b8da]" title="Edit"><Edit size={16} /></button>
                ...
                <div className="w-full bg-white border shadow-sm p-8 relative text-sm print:w-full print:border-none print:shadow-none print:p-0 min-h-[1000px]">
                    ...
                    <div className="flex justify-between items-start border-t border-b border-gray-300 py-4 mb-6">
                        <div className="space-y-1">
                            ...
                            <div className="flex"><span className="w-32 font-semibold">Reference No.</span><span>: {invoice.reference || '-'}</span></div>
                            <div className="flex"><span className="w-32 font-semibold">Order No.</span><span>: {invoice.orderNumber || '-'}</span></div>
                        ...
                        <div className="text-right">
                            ...
                            <div className="mt-4 bg-gray-100 p-2 rounded inline-block text-left">
                                ...
                                <span className="block text-xl font-bold text-red-600">AED {invoice.balanceDue?.toFixed(2)}</span>
                            </div>
                        </div>
                    </div>
                        ...
                        <tbody className="text-xs">
                            {invoice.items?.map((item, index) => (
                                <tr key={index} className="border-b border-gray-200">
                                    ...
                                    <td className="py-3 text-center">{item.invoiceQuantity || item.quantity}</td>
                    ...
                    <div className="flex flex-col md:flex-row gap-8">
                        <div className="flex-1 space-y-6">
                            ...
                            {invoice.bankDetails && (
                                <div>
                                    <h4 className="font-bold text-sm mb-1">Bank Details</h4>
                                    <p className="text-xs text-gray-600 whitespace-pre-wrap">{invoice.bankDetails}</p>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="w-full md:w-80">
                            ...
                            <div className="flex justify-between py-2 text-base font-bold border-t border-gray-300 mt-2 text-red-600">
                                ...
                                <span>AED {invoice.balanceDue?.toLocaleString('en-AE', { minimumFractionDigits: 2 })}</span>
                            </div>
                        </div>
                    </div>
